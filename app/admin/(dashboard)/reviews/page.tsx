    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_#2A103D,_#160B24,_#0B0612)] text-white p-6 md:p-10 relative">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <section className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <button 
            ...
            className="rounded-2xl border border-accent-gold/30 bg-accent-gold/10 px-6 py-3 text-accent-gold hover:bg-accent-gold/20 transition-all shadow-lg font-medium"
          >
            + 후기 직접 등록
          </button>
        </section>
                  ...
                  {reviews.map((review) => (
                    <tr key={review.id} className="hover:bg-white/5 transition-colors group">
                      ...
                      <td className="p-5">
                        ...
                        <p className="text-xs text-accent-gold/70 mt-1">{renderStars(review.rating)}</p>
                      </td>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-xl rounded-3xl border border-accent-gold/30 bg-[#1A0B2E] p-8 shadow-2xl relative overflow-hidden">
            {/* 배경 장식 */}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex gap-6">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input 
                    type="checkbox" 
                    className="w-5 h-5 rounded border-white/10 bg-white/5 checked:bg-accent-gold"
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button 
                  ...
                  className="flex-1 rounded-xl bg-accent-gold text-purple-900 py-3 font-semibold hover:bg-accent-gold/80 transition-all disabled:opacity-50"
                >
